// Copyright (c) 2012 WiredTiger, Inc.  All rights reserved.
...
inline
static void DBSynchronize(WT_SESSION *s)
{
  int ret;
...
class Benchmark {
  ...
  : conn_(NULL),
    ...
    if (!FLAGS_use_existing_db) {
      for (int i = 0; i < files.size(); i++) {
        //if (Slice(files[i]).starts_with("dbbench_wt")) {
 ...
 private:
    ...
    conn_->open_session(conn_, NULL, NULL, &session_);
    ...
    config << ",internal_page_max=" << FLAGS_page_size;
    config << ",leaf_page_max=" << FLAGS_page_size;
    if (FLAGS_compression) {
      config << ",block_compressor=snappy";
    }
    /* XXX: not handling sync. */
    (void)sync;

    int ret = session_->create(session_, uri_.c_str(), config.str().c_str());
    if (ret != 0) {
  ...
  void Write(bool sync, Order order, DBState state,
    ...
    WT_CURSOR *cursor;
    int ret = session_->open_cursor(session_, uri_.c_str(), NULL, "overwrite", &cursor);
    if (ret != 0) {
    ...
    for (int i = 0; i < num_entries; i++)
    {
      ...
      std::string value = gen_.Generate(value_size).ToString();
  ...
  void ReadSequential() {
    ...
    WT_CURSOR *cursor;
    int ret = session_->open_cursor(session_, uri_.c_str(), NULL, NULL, &cursor);
    if (ret != 0) {
      fprintf(stderr, "open_cursor error: %s\n", wiredtiger_strerror(ret));
      return;
    }
    ...
    while (cursor->next(cursor) == 0) {
      ...
      bytes_ += strlen(ckey) + strlen(cvalue);
      FinishedSingleOp();
    }
